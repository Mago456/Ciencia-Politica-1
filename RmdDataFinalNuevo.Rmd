...
```{r}
Sis[,c(3,4,5,7)]=NULL
names(Sis) = c("Provincia", "Regiones", "%Si-tiene-seguro")
```
...
names(ProvinciasOrdenado) = c("Provincia", "Regiones", "%Si-tiene-acceso-al-agua")
```
...
#renombramos
```{r}
names(DataInternet) = c("Provincia", "Regiones", "%Si-tiene-acceso-a-internet")
```
...
```{r}
names(Alfordenado) = c("Provincia", "Regiones", "%Si-sabe-leer-y-escribir")
```
...
```{r}
setwd("C:/Users/W10/Desktop/Clase 6")
```
